...
# generate sentence IDs in a given range based on a template ID
def generate_sentence_ids(template_id, start, end):
    local_nr = 150
    global_nr = 21339
    return [f'{template_id},{local_nr + i}.{global_nr + i}' for i in range(start, end)]
...
def assert_correct_word_class(token, tag):
    if tag in  ".,:;!?()[]{}«»–—„“”‘’…" and token.upos == "PUNCT":
        tag = tag
    elif not token.upos in lex_class_map[tag[0]]:
        tag = "x"
    return tag
...
if __name__ == "__main__":
    ...
    file_path = "../../is_icepahc-ud-train.conllu"
    
    SKALHOLT_IDS = generate_sentence_ids("1680.SKALHOLT.NAR-REL", 0, 74)
    ...
    for sentence in conll:
        runner += 1
        if not sentence.id in SKALHOLT_IDS:
            if runner % 10 == 0:
        ...
        for token in sentence:
            # guard clause to not handle tokens with joined clitics
            if "-" in token.id:
                continue
            else:
                token = update_token_features_and_misc(token, tag_dict)
